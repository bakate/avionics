...
export const BookingItem = ({ booking, index }: BookingItemProps) => {
  ...
  return (
    <Link to={buildRoute.bookingDetails(booking.pnrCode)} key={booking.id}>
      <Item
      ...
      >
        <ItemMedia
          variant="image"
          className="bg-slate-800/50 overflow-hidden relative"
        >
          {!imageLoaded && (
          ...
          <img
            ...
            onError={() => {
              if (!hasError) {
                ...
                // Even default image failed, stop loading to show nothing/background
                setImageLoaded(true);
              }
            }}
          />
