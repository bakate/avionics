...
// Actor
// ---------------------------------------------------------------------------
const executeStressTest = fromPromise(
  async ({
    input,
  }: {
    ...
    // Create requests
    const requests = Array.from({ length: input.requestCount }).map((_, i) => {
    ...
    const eithers = await Effect.runPromise(
      Effect.all(requests, { concurrency: "unbounded" }),
    );
...
const fetchAvailableFlights = fromPromise(async () => {
  ...
  return result.map((f) => ({
    ...
    origin: f.origin.valueOf() ?? "",
    destination: f.destination.valueOf() ?? "",
    departureTime:
...
const fetchSingleFlight = fromPromise(
  async ({ input }: { input: { flightId: string } }) => {
    const f = await Effect.runPromise(getFlightAvailability(input.flightId));
        ...
        onDone: {
          actions: assign({
            flights: ({ context, event }) =>
              context.flights.map((f) =>
                f.flightId === event.output.flightId ? event.output : f,
              ),
          }),
        },
      },
